...
- **M3 마찰 완화** 🔄 진행 중 (2026-08-04 시작): 승인 채널을 AskUserQuestion 선택창으로 전환(D15), 발산 0은 압축 확인 단일 왕복(D16). 훅 3종이 공통 `resolveAskApproval`로 선택 승인을 판정 — 쓰기 직전·턴 종료·다음 발화 어디서든 전이돼 데드락 원천 차단. 구현·테스트 완료(신규 12건 포함 전체 통과), 재설치 dogfooding으로 검증 예정 — `docs/m3-friction-notes.md`. **재설치의 선결 조건**
