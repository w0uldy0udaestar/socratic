...
- **M0 실측 스파이크** (최우선): 리서치는 전부 문서 기반 — Claude Code에서 `UserPromptSubmit` 주입·`PreToolUse` 차단·30초 타임아웃·상태 파일 공유를 최소 스크립트로 실측 검증. **+ `Stop` 훅으로 비파일 작업(글쓰기·리서치 답변)의 "의도 확인 전 종료 차단"이 가능한지 실측 (D11)**
